...
@HiltViewModel
class MainViewModel @Inject constructor(private val getCityUseCase: GetLastSearchedCityNameUseCase) :
    ViewModel() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            when (val result = getCityUseCase()) {
                is BaseState.Success -> {
                    _cityName.value =
                        if (!result.data.isNullOrEmpty()) BaseState.Success(result.data!!)
                        else BaseState.Failure(null)
                }
